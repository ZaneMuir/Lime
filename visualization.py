from special_items import chart_dir, eye_data_path, eyed_span
...
def R_raw(ch_num, data, on_thresh, bout_time_pair, scatter_result, isEyed=False, title=None, inline=False):
    ...
    if isEyed: #添加人工识别的结果
        ...
        for index in eye.index:
            plt.plot(eye.iloc[index].tolist(), np.ones(eye.iloc[index].shape)*on_thresh*(-2),c='black',linewidth=3)
...
def exportCSV(ch_num, bout_time_pair, scatter_result,title=None):
