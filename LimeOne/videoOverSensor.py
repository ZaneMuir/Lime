...
def mainDB(sessionID, dbCursor, duration, output):
    ...
    for each in sessionID:
        ...
        climb = pd.DataFrame(dbCursor.fetchall(),
                             columns=['n', 'start', 'end'])
        ...
        temp = pd.DataFrame(np.hstack((times[:, np.newaxis],
                            boutarray[:, np.newaxis])),
                            columns=['time', 'bout'])
        chewing_result = [(index, item[0], item[-1]) for index, item in
                          enumerate(
                    group_consecutive(temp[temp.bout == 1]['time'].values))]
...
def main(ChewFilePath, EyeFilePath, outputName=None, arguments=None):
    ...
    plt.xlim((0, 3600))
    if outputName:
        plt.savefig(os.path.join(outputName, 'chewing_plot.png'),
                    bbox_inches='tight')
    else:
        plt.show()
