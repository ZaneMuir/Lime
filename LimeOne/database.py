...
def createNewPoseTable(cursor, session_name, title, pose_info):
    u"""
    建立新的视频分析结果表格，仅储存每帧ROI内的像素灰度均值.

    createNewPoseTable(cursor::sqlite3.cursor,
        session_name::string,
        title::list, pose_info::list)

    cursor:         数据库cursor
    session_name:   session的序号，即{DATE}{ID}
    title:          表头，应与ncage个数相对应，每只老鼠的title为p{ID},
                    如，ncage=4时，title=['p1','p2','p3','p4']
    pose_info:      视频的原始数据，列表中每个元素应为list或者tuple，
                    每个元素应包含: (frame::int, time::real, p1::real, p2::real, ...)
    """
    try:
        # 创建表格
        cursor.execute("""
    ...
    # 生成需要执行的sql命令
    command = "INSERT INTO %s VALUES (%s)" % (session_name,
                                              "%d"+",%.3f"*(len(title)+1))
...
def createNewSessionTable(cursor, session_name, title, session_info):
    ...
    createNewSessionTable(cursor::sqlite3.cursor,
                          session_name::string,
                          title::list, session_info::list)

    表格主要用来保存攀爬与啃食的起始与终止时刻信息，
    session_info中的每个元素应包含:(id::int, start::real, end::real)
    """
    try:
        cursor.execute("""
            CREATE TABLE %s (
...
def createNewSessionSummaryEntry(cursor, sessionID, session_summary_info):
    ...
    createNewSessionSummaryEntry(cursor::sqlite3.cursor,
                                 sessionID::list,
                                 session_summary_info::list)
    """
    for index, each in enumerate(sessionID):
        ...
        command = """
            INSERT INTO summary VALUES ("%s",
            "%s",%d,"%s","%s","%s",%d,%d,"%s",
            %f,"%s",%d,%.3f,%d,%.3f,%.3f,%.3f,
            %.3f,%.3f,%.1f,"%s")""" % (tuple(session_summary_info[index]))
