...
def poseCheck_DB(dbCursor, session_name, sessionID, offset, output='.', ncage=2, scale = 1):
    ...
    # 转化为DataFrame，之所以没有用pandas自带的sql读取，是因为pandas有时候会有一些奇奇怪怪的meta信息出来
    ...
    for each in range(ncage): # 对每只老鼠做以下操作：
        ...
        createNewSessionTable(dbCursor, sessionID[each]+"_Climb", title, targetList) # 创建新的表格

        posePlot(os.path.join(output,sessionID[each]+"_Climb.png"),thresh, array) # 绘制中间信息的图像
    pbar.close()
