...
void ProgressBar::Progressed(unsigned long idx_)
{
    try{
        ...
        double remaining = (double(std::time(nullptr) - start) / progress_percent * (TOTAL_PERCENTAGE - progress_percent)) / 60.0;
        ...
        *out << "]" << std::setw(CHARACTER_WIDTH_PERCENTAGE + 1) << std::setprecision(1) << std::fixed << progress_percent<< "\t" << remaining << " min.\r" << std::flush;
    }
